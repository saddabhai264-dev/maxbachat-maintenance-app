...
async function uploadCompressedPhoto(file){
  ...
  const uploaded = await apiFetch('/media/upload', {
    method:'POST',
    body: JSON.stringify({
      filename: (file.name || `photo-${Date.now()}.jpg`).replace(/\.[^.]+$/, '.jpg'),
...
function openDeleteIssueModal(id){
  const i = ISSUES.find(x=>x.id===id);
  if(!i) return;
  const adminNote = currentUser && currentUser.role === 'admin'
    ? 'Admin access can delete open, verified, or closed issues. This action cannot be undone.'
    : 'This will remove the issue and its uploaded proof from the dashboard. Use this only when the issue was added by mistake.';
  showModal(`
    <div class="modal-head"><h3>Delete issue</h3><button class="x-btn" onclick="closeModal()">&times;</button></div>
    <p class="sub">${escapeHtml(i.title)}</p>
    <div class="proof-note">${adminNote}</div>
    <div class="modal-actions">
      <button class="btn" onclick="closeModal()">Cancel</button>
